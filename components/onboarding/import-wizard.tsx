...
export type ImportModulo = 'pacientes' | 'financeiro' | 'registros'
...
const MODULOS_CFG: Record<ImportModulo, {
  label: string
  ...
  pacientes: {
    label: 'Pacientes',
    icon: Users,
    colunas: ['nome', 'cpf', 'data_nascimento', 'telefone', 'email', 'status', 'responsavel', 'data_anamnese', 'data_inicio', 'cep', 'logradouro', 'numero', 'complemento', 'bairro', 'cidade', 'estado'],
    cor: 'text-[#04c2fb]',
    corBg: 'bg-[#04c2fb]/10',
...
const COLUNAS_INFO: Record<ImportModulo, ColInfo[]> = {
  pacientes: [
    ...
    { campo: 'status',         desc: '"ativo" ou "inativo" (padrão: ativo)',                obrigatorio: false },
    ...
    { campo: 'cep',            desc: 'CEP no formato 00000-000 ou 00000000',                obrigatorio: false },
    { campo: 'logradouro',     desc: 'Rua, avenida ou endereço principal',                  obrigatorio: false },
    { campo: 'numero',         desc: 'Número do imóvel',                                   obrigatorio: false },
    { campo: 'complemento',    desc: 'Apto, sala, bloco (opcional)',                        obrigatorio: false },
    { campo: 'bairro',         desc: 'Bairro',                                             obrigatorio: false },
    { campo: 'cidade',         desc: 'Cidade',                                             obrigatorio: false },
    { campo: 'estado',         desc: 'UF com 2 letras, ex: PR',                            obrigatorio: false },
  ],
  financeiro: [
    { campo: 'tipo', desc: '"receita" ou "despesa"', obrigatorio: true },
    ...
    { campo: 'data_referencia', desc: 'Mês de competência no formato MM/AAAA', obrigatorio: false },
    { campo: 'data_pagamento', desc: 'Data do pagamento (DD/MM/AAAA)', obrigatorio: false },
...
export function ImportWizard({ modulo, onSuccess }: ImportWizardProps) {
  const cfg = MODULOS_CFG[modulo]
  const colunasInfo = COLUNAS_INFO[modulo]
  const { label: moduloLabel, colunas } = cfg
                  ...
                  <React.Fragment key={col.campo}>
                    <Badge
                      ...
                      className={cn(
                        ...
                        col.obrigatorio ? 'bg-[#04c2fb]/15 text-[#0094c8] border-0' : 'text-slate-500',
                      )}
                    >
                      {col.campo}
                    </Badge>
                    <span className="text-xs text-slate-500">
                      {col.desc}
                      {col.obrigatorio && <span className="ml-1 text-[#04c2fb] font-medium">obrigatório</span>}
                    </span>
                  </React.Fragment>
                ))}
              </div>
            ...
            <div className="pl-8">
              <div
                ...
                className={cn(
                  ...
                  dragging ? 'border-[#04c2fb] bg-[#04c2fb]/5' :
              ...
              >
                ...
                {file ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-slate-400 text-center">Clique aqui ou arraste o arquivo para esta área</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
                ...
                <div className="rounded-md bg-amber-100/60 border border-amber-200 px-3 py-2.5 space-y-2">
                  ...
                  <ul className="pl-3 space-y-0.5">
                    {analise.pacientesNaoEncontrados.map((nome) => (
                      <li key={nome} className="text-xs font-medium text-amber-800 list-disc">{nome}</li>
                    ))}
                  </ul>
                </div>
              )}
              {analise.pacienteNomeAusente > 0 && (
                <div className="rounded-md bg-amber-100/60 border border-amber-200 px-3 py-2.5 space-y-1">
                  ...
                  <p className="text-xs text-amber-700">
                    A coluna <span className="font-mono font-medium">paciente_nome</span> é obrigatória e está vazia nessas linhas.
                  </p>
                </div>
              )}
              {analise.nomesAmbiguos.length > 0 && (
                <div className="rounded-md bg-amber-100/60 border border-amber-200 px-3 py-2.5 space-y-2">
                  <p className="text-xs font-semibold text-amber-900">Nome com mais de um paciente cadastrado</p>
                  <ul className="pl-3 space-y-0.5">
              ...
              {analise.formatosInvalidos > 0 && (
                <div className="rounded-md bg-amber-100/60 border border-amber-200 px-3 py-2.5 space-y-1">
                  ...
                  <p className="text-xs text-amber-700">
                    Datas: <span className="font-mono font-medium">DD/MM/AAAA</span>, valores: <span className="font-mono font-medium">200,00</span>
                  </p>
                </div>
              )}
            </div>
            <p className="text-xs text-amber-600">As linhas com problema serão ignoradas na importação.</p>
          </div>
        )}
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    ...
                    {colunas.map((c) => (
                      <th key={c} className="px-3 py-2 text-left text-slate-500 font-medium whitespace-nowrap font-mono">{c}</th>
                    ))}
                ...
                <tbody>
                  {preview.rows.map((row) => (
                    <tr key={row.linha} className={cn('border-b border-slate-100 last:border-0', row.valido ? 'bg-white' : 'bg-red-50')}>
        ...
        <CardContent className="py-10 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            ...
            {resultado.erros.length > 0 ? (
              ...
              <p className="text-xs text-slate-500 mt-1">Todos os registros foram importados sem problemas.</p>
            )}
          </div>
          {resultado.erros.length > 0 && (
            <div className="w-full max-w-md rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-left space-y-1">
              ...
              {resultado.erros.map((e, i) => (
                <p key={i} className="text-[10px] text-red-600">Linha {e.indice + 1}: {e.erro}</p>
              ))}
            </div>
          )}
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            {onSuccess && (
              <Button
                size="sm"
                onClick={onSuccess}
                className="gap-1.5 text-xs text-white hover:brightness-110"
                style={{ background: 'linear-gradient(135deg, #0094c8 0%, #04c2fb 60%, #00d5f5 100%)' }}
              >
                Continuar
              </Button>
